...
class DamFrontendConverter
{
    ...
    protected function getFalCategory($damCatUids)
    {
        ...
        return Tx_Rnbase_Database_Connection::getInstance()->doSelect('uid,title,description',
            'sys_category', $options);
    }
...
//		}
        ...
        $colId = Tx_Rnbase_Database_Connection::getInstance()->doInsert('sys_file_collection',
            $data);
    ...
    private function findFileCollectionByCategory($categoryUid)
    {
        ...
        return reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('uid',
            'sys_file_collection', $options));
    }
    ...
    private function fixTemplaVoilaPageEntry($row, $fileLinksUid)
    {
        ...
        $page = reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('tx_templavoila_flex',
            'pages', $options));
        ...
        $templavoilaFlex = StringUtility::addUidStr($page['tx_templavoila_flex'], $row['uid'],
            $fileLinksUid);

        $this->debug("\nflexVOR: " . $page['tx_templavoila_flex'] . "\nfelxNACH: $templavoilaFlex\n\n");

        return Tx_Rnbase_Database_Connection::getInstance()
            ->doUpdate('pages', $options['where'],
                array('tx_templavoila_flex' => $templavoilaFlex));
    }
        ...
        foreach ($filteredDamRecordUids as $damRecordUid) {
            ...
            if (!$falUid) {
                ...
                if ($fileLinksUid !== false) {
                    $refid = \tx_rnbase_util_TSFAL::addReference('tt_content', 'media',
                        $fileLinksUid, $falUid, $item['pid']);
    ...
    private function fixTemplaVoilaFileLinkEntryInFce($row, $fileLinksUid)
    {
        ...
        foreach ($fces AS $fce) {
            if (strstr($fce['tx_templavoila_flex'],
                    $fileLinksUid) !== false) { // do not insert same id twice
            } else {
                $templavoilaFlex = StringUtility::addUidStr($fce['tx_templavoila_flex'],
                    $row['uid'], $fileLinksUid);
                ...
                Tx_Rnbase_Database_Connection::getInstance()
                    ->doUpdate('tt_content', $options_a['where'],
                        array('tx_templavoila_flex' => $templavoilaFlex));
            }
        }
        return true;
    ...
    protected function getFalUidFromDam($damUid)
    {
        ...
        $result = reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('uid', 'sys_file',
            $options_a));
        $result = ($result && array_key_exists('uid', $result)) ? $result['uid'] : false;
        //if no result try to get from tx_dam table
        if (!$result) {
            $result = reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('falUid',
                'tx_dam', $options_b));
    ...
    private function getDamTitle($damId)
    {
        ...
        $result = reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('title', 'tx_dam',
            $options));
    ...
    private function getDamDescription($damId)
    {
        ...
        $result = reset(Tx_Rnbase_Database_Connection::getInstance()->doSelect('description',
            'tx_dam', $options));
        return ($result && array_key_exists('description',
                $result)) ? $result['description'] : false;
    }
    ...
    private function getFilelinkPluginUid($row)
    {
        ...
        $result = Tx_Rnbase_Database_Connection::getInstance()->doSelect('uid', 'tt_content',
            $options);
        if (sizeof($result) > 0) {
            $return = array();
