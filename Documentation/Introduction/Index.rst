...
As this is a fork of the we_dam2fal extension we provide some additional features.

- it's not only possible to migrate files which reside in the fileadmin folder but in every possible storage
- in the extension configuration it can be configured how many datasets are migrated per submit. With we_dam2fal the limit are 10000 datasets per submit
